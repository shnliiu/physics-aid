...
export default function ChapterPage({
  ...
  const fetchChapterData = async () => {
    try {
      ...
      setFormulas([
        { id: '1', title: 'Mock Formula', latex: 'E = mc^2', description: 'Energy-mass equivalence' }
      ]);
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="border-b border-gray-200 dark:border-gray-700 mb-6">
          <nav className="flex gap-4">
            {(['formulas', 'problems', 'chat'] as TabType[]).map((tab) => (
              <button
                key={tab}
                onClick={() => setActiveTab(tab)}
                className={`px-4 py-2 border-b-2 font-medium text-sm transition-colors ${activeTab === tab
                  ...
                  {problems.map((problem) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 text-xs">
                        <span
                          className={`px-2 py-1 rounded ${problem.status === 'SOLVED'
